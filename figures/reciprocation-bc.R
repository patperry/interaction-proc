# figures/reciprocation-bc.R
# --------------------------

source("code/boot.R")


pdffile <- "figures/reciprocation-bc.pdf"
pdfout <- TRUE
...
bias <- GetBias()
coefs <- coefs - bias$est[seq_along(coefs)]
coefs.se <- sqrt(coefs.se^2 + (bias$est.se[seq_along(coefs)])^2)
