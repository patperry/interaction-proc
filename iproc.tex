...
We use the proportional intensity model with multiple recipients from
Section~\ref{S:multiple-recipients} to model the Enron data.
Our analysis excludes messages with more than 10 recipients---a
...
a bias-correction to be described in Sections (FIXME: refs).  For standard
error methods, we use the forthcoming theory in the next two sections
and employ the delta method where appropriate.
...
\begin{table}[h]
    ...
    \caption{
        ...
        at the company ($0.276$ and $0.724$, respectively).
    }
    \label{T:gender-effects}
\end{table}

According to Table~\ref{T:gender-effects}, females are biased
towards sending to females, and males are biased towards sending to
males, even after adjusting for reciprocation effects.  However,
the bias is not as strong as suggested by the naive estimates from
Section~\ref{S:motivating-example}.
...
\section{Bias Correction for Multiple Recipients}
...
$\log \widetilde{\mathit{PL}}.$  When $|\mathcal{J}_{t_m}(i_m)|$ is
...
\section{Estimates without Bias-Correction}

This section gives raw estimates of the parameters in
the Enron model, without bias correction.

\begin{table}[h]
    \label{T:gender-effects-comparison}
    ...
    \subtable[Adjusted for Reciprocation]{
        ...
        \label{T:gender-effects-static-dynamic}        
    }
    \subtable[Adjusted for Multiple Recipients]{
        \input{tables/gender-effects-dynamic-bc}
        \label{T:gender-effects-static-dynamic-bc}        
    }
    \caption{
        blah blah
    }
