...
\begin{abstract}
Network data often take the form of counts of repeated interactions over time.  Here we consider partial-likelihood inference for the general case of \emph{directed} interactions involving senders and receivers, communications data being a canonical example.  We model these data in continuous time by way of a point process representation, using a Cox multiplicative intensity model that takes into account covariates that depend on the history of the process.  We prove consistency and asymptotic Normality of our estimation procedure as time increases, in contrast to the fixed-interval setting typical of the clinical trials literature.  We also address the issue of consistent inference for interactions with multiple receivers, which has been largely ignored in the network literature to date.  We show that treating these as multiple separate interactions leads to biased estimators, and we give conditions under which this bias can be corrected. We also describe an efficient implementation of the resulting inference and bootstrap procedures, and conclude with a data analysis example showing the effects of reciprocity and group-level preferences on message sending behavior in a corporate e-mail dataset.
\end{abstract}
...
\begin{keyword}
    \kwd{Cox proportional hazards model}
    \kwd{network data analysis}
    \kwd{partial likelihood inference}
    \kwd{point processes}
\end{keyword}
...
Much effort has recently been devoted to the statistical analysis of network data; see~\cite{goldenberg2009survey,jackson2008social,kolaczyk2009statistical} for recent overviews.  While such data are typically treated as ``ties'' between entities, it is often the case in practice that network observables comprise counts of interactions between individuals or groups tabulated over time.  Modern-day communications networks give rise to important examples of \emph{directed} interactions: phone calls, text messages, or e-mails exchanged amongst a given set of individuals over a specific time period \cite{eagle2006reality,tyler2005email}.  Other examples of tabulated interactions include animal association patterns (for instance, zebras congregate at locations in their habitat \cite{sundaresan2007network}); gang homicides (e.g., gangs in Chicago murder members of rival factions \cite{papachristos2009murder}; legislation cosponsorship (legislators author bills, which others then sponsor \cite{fowler2006connecting}); and migration patterns (for example, families migrate between communities in Mexico \cite{mckenzie2007network}).

In this article, we consider partial-likelihood inference for the general case of directed interaction data in the presence of covariates.  We first develop asymptotic theory for the case in which interactions are strictly pairwise, and then generalize our results to the multiple-interaction case; we also provide efficient algorithms for partial likelihood maximization in these settings.  Our main assumptions are that the interactions are instantaneous; that the corresponding stochastic intensities used to model them are formally predictable and continuous; and that any covariates are locally bounded and predictable.

The interaction data we consider comprise a set of triples $(t,i,j)$ indicating that at time $t$, the directed interaction $i\rightarrow j$ took place; for instance, individual $i$ sent a message to individual $j$.  Given a set of such triples and covariate information about the corresponding senders and receivers, a primary modeling goal is to determine which characteristics and behaviors are predictive of interaction.  In this vein, three important questions stand out; the first is a longstanding one in studies of human behavior~\cite{mcpherson2001birds}, whereas the latter two have yet to receive due attention in the network literature:

\begin{description}
    \item[Homophily] Is a shared attribute predictive of increased
    interaction between entities?  Is evidence of homophily---a heightened rate of interaction among similar individuals---present?

    \item[Reciprocity] Are past interactions $i \rightarrow j$ predictive of future reciprocal interactions $j \rightarrow i$? If so, how might this effect decay with time?

    \item[Multiplicity] How should multiple-recipient interactions of the type $i \rightarrow \{j_1,j_2,\ldots\}$ be modeled?  What are the effects of treating these as separate pairwise interactions?
\end{description}

In the remainder of this article, we provide a modeling framework and computationally efficient partial-likelihood inference procedures to facilitate an analysis of these questions.  We employ a Cox proportional intensity model incorporating both static and history-dependent covariates to address the first of these two questions, and a parametric bootstrap to address the third.  Section~\ref{S:point-process-model} presents our point process model for directed pairwise interactions, along with the resultant inference procedures.  Section~\ref{S:MPLE-consistency} proves consistency and asymptotic Normality of the corresponding maximum partial likelihood estimator, and Section~\ref{S:multiple-receivers} extends our framework to the case of multiple-receiver interactions.  A data analysis example is presented in Section~\ref{S:enron-modeling}, and Section~\ref{S:discussion} concludes with a brief discussion.  Details of implementation and technical lemmas are provided in the appendices.
...
Efron handles them through an ad-hoc adjustment, while Brostr\"om adds a
...
be an unknown vector of coefficients in  $\reals^p$.  For the remainder of this section,
assume that each interaction has a single receiver.
...
expressed in terms of a weighted mean and covariance of the covariates.  The weights are
\begin{subequations}
\begin{align}
    w_{t}(\beta, i,j)
        &=
...
$V_{t}(\cdot, i)$, given respectively by
\begin{subequations}
\begin{gather}
    E_{t}(\beta, i)
        =
...
Because the introduction of time-varying covariates precludes the use of sufficient statistics, 
partial likelihood maximization for all but the smallest interaction datasets 
requires an algorithm that is more efficient than
standard tools from survival analysis (e.g., the function 
\texttt{coxph} from the \texttt{survival} package for R
\cite{therneau2009survival}).  We describe in  Appendix~\ref{S:implementation} a custom gradient-based
optimization method that takes advantage
of sparsity in $x_t(i,j)$.
...
observe more data is to increase $t$.  Cox \cite{cox1972regression} outlines a proof for general
...
time-varying covariates \cite{cox1975partial,wong1986theory}. Thus, the general
interaction data sampling regime mandates a new consistency proof.

Despite assuming $\mathcal{I}$ and $\mathcal{J}$ to be fixed, our analysis
...
\)
also define the sequence of stopping times
...
\begin{theorem}\label{T:score-fisher}
    ...
    vector $\beta_0$.  Let $\tau_n$ be the sequence of interaction times
...
\begin{theorem}\label{T:consistency}
    ...
    Let $\tau_n$ be the sequence of interaction times defined
...
hypothesis of the theorem and $\{ V_{\tau_{n'}}(\cdot, i) \}$ is an
equicontinuous family by assumption~A\ref{A:var-equicont}.
Lemma~\ref{L:Lenglart} proves, as a consequence of
assumption~A\ref{A:message-times-finite} and Lenglart's
Inequality~\cite{lenglart1977relation}, that the second term converges to zero
uniformly in $\alpha$.
...
do not describe interactions that can involve multiple receivers (e.g., e-mail
...
likelihood for our model.  In particular we show this approximation to be closely
related to an extension of the model, and that the estimator bias it
introduces can be quantified and in certain cases corrected.
...
Let $(t_1, i_1, J_1), \ldots, (t_n, i_n, J_n)$ in turn be the
...
sum over all sets of size $|J_m|$ in~\eqref{E:log-pl-multiple} with a sum over all multisets of size
$|J_m|$ allowing duplicate elements, we observe
\begin{align*}
    \log\big[\!\!
        \sum_{\substack{J \subseteq \mathcal{J}_{t_m}(i_m) \\
              |J| = |J_m|}}\!\!\!
...
Note that $G_n$ has linear growth when $|\mathcal{J}_{t_m}(i_m)|$ is constant,
but that when $|\mathcal{J}_{t_m}(i_m)|$ increases, $G_n$ often has sublinear
...
\[
    G_n
        \leq
            ...
            \bigg]^{1/2},
\]
and so if $|\mathcal{J}_{t_m}(i_m)| = \omega(\sqrt{m})$, then
...
\end{gather*}
\end{subequations}
and note that the approximation error in $\log \widetilde{\mathit{PL}}_t(\beta)$ comes from replacing
$W$ with $\widetilde W$.
...
\]
We may then adjust for estimator bias by replacing $\tilde \beta_n$ with
$\tilde \beta_n - \widehat{\mathrm{bias}}$.


\section{Analysis of a Corporate E-mail Network}\label{S:enron-modeling}

We now consider a data analysis example to demonstrate the efficacy of the models and inference methods introduced above, and to show how they enable us to address the three key concerns of interaction data analysis (homophily, reciprocity, and multiplicity) raised in the Introduction of this article.  To do so we employ the multiple-recipient proportional intensity modeling framework developed above; we include static covariates reflecting group membership to account for the possibility of homophily, and dynamic covariates capturing past interactions to measure reciprocity effects.

\subsection{Data and Methods}

Our example analysis uses publicly available data from the Enron e-mail corpus~\cite{cohen2009enron}, a large subset of the e-mail messages sent within the Enron corporation between November 1998 and June 2002, and made public as the result of a subpoena by the U.S.~Federal Energy Regulatory Commission during an investigation into fraudulent accounting practices.  We analyze the dataset compiled by~\cite{zhou2007strategies}, comprising $21,635$ messages sent among $156$ employees between November 13, 1998 and June 21, 2002, along with the genders, seniorities, and departments of these employees (summarized in Table~\ref{T:employee-summary}).
\begin{table}[t]
    \input{tables/employee-summary}
    \caption{
        Characteristics of the 156 employees in the Enron dataset~\cite{zhou2007strategies}.
    }
    \label{T:employee-summary}
\end{table}
Approximately 30\% of these messages have more than one recipient across their \texttt{To}, \texttt{CC}, and \texttt{BCC} fields, with a few messages having more than fifty recipients (see Fig.~\ref{F:recipient-counts}).
\begin{figure}
    ...
    \caption{
        ...
        $y$-value showing the base-2 logarithm of its number of recipients (ranging from 1--57).
    }\label{F:recipient-counts}
\end{figure}
In the subsequent analysis, we exclude messages with more than 10 recipients---a subjectively-chosen cutoff at the 99th percentile that avoids e-mails sent \emph{en masse} to large groups.

We model these data using the multiple-recipient proportional intensity model of Section~\ref{S:multiple-receivers}, with $\mathcal{I} = \mathcal{J} = \{ 1, 2, \ldots, 156 \}$ and $\mathcal{J}_t(i) = \mathcal{I} \setminus \{ i \}$, and with static and dynamic covariates taking the form of indicator variables as described below.  We fit the model by first maximizing the approximate log partial likelihood of~\eqref{E:log-pl-multiple-approx}, and then employing a parametric bootstrap to estimate and correct the resultant bias in parameter estimates.  Standard errors are calculated using the corresponding asymptotic theory, along with the delta method as appropriate.  Note that in the setting of this data analysis example, the covariates are bounded and the interaction count is high, and so assumptions A\ref{A:square-int}--A\ref{A:var-equicont} and the asymptotic framework developed in Sections~\ref{S:MPLE-consistency} and~\ref{S:multiple-receivers} are natural.

\subsection{Covariates and Identifiability}

For each $i \in \mathcal{I}$ and $j \in \mathcal{J}_t(i)$, we employ dyad covariate vectors $x_t(i,j) \in \{0,1\}^{165}$, each with $144 + 21$ binary components, corresponding respectively to static and dynamic regressors.  The former are determined by the sender- and receiver-specific group memberships summarized in Table~\ref{T:employee-summary}: abbreviating the codes for gender, department, and seniority by F/M, L/T/O, and J/S, the $144$ static components of $x_t(i,j)$ are obtained by stacking the columns of $s(i) \otimes s(j)$, where
\begin{multline*}
    s(i)
    = \big(
        \text{1\{$i$ is FLJ\}},
    ...
    \big)
\end{multline*}
is the group membership vector for individual $i$.  The $21$ dynamic components of $x_t(i,j)$ are history-dependent and designed to capture reciprocity effects; they take the form
\[
    1\{\text{$j$'s most recent message to $i$ was sent in $[t-2^k \text{ hours}, t)$\}},
\]
where $k$ ranges from $-6$ to $14$, yielding time intervals from $56.25$ seconds to $1.87$ years.  (Effects of intervals outside of this range could not be estimated, with significance, from the available data.)

Note that the components of $\beta_0$ corresponding to the static covariates are not identifiable, since any $\tilde \beta_0$ satisfying $\tilde \beta_0^\trans x_t(i,j) = \beta_0^\trans x_t(i,j) + \alpha_t(i)$ gives rise to the same partial likelihood.  We identify $\beta_0$ by by imposing ``sum-to-zero'' constraints within each of the $12$ receiver classes, reducing the corresponding degrees of freedom from $144$ to $132$.

\subsection{Results}

Given the model specification, data, and covariates outlined above, we may estimate the parameter vector $\beta_0$ under the approximate log partial likelihood of~\eqref{E:log-pl-multiple-approx}.  Recall that the results of Section~\ref{S:multiple-receivers} bound the bias resulting from this approximate MPLE procedure as a function of the growth rate of the recipient set $\mathcal{J}$ over time.  Here, treating the set $\mathcal{J}$ of 156 Enron employees as constant, the resultant bias is of order $1/|\mathcal{J}|$---and, since $|\mathcal{J}| = 156$ is on the order of the square root of the number $21,365$ of messages in the dataset, we can correct this bias using the parametric bootstrap outlined at the end of Section~\ref{S:multiple-receivers}.  Figure~\ref{F:boot-resid}
\begin{figure}[t]
    \includegraphics[scale=0.6]{figures/boot-resid}
    \caption{
        \textsc{Enron Bootstrap Residuals.} Summary of bootstrap residuals for
        estimated coefficients using the Enron dataset, normalized by estimated
        ...
        145--165 are the reciprocity (dynamic) effects, ordered from
    ...
    \label{F:boot-resid}
\end{figure}
summarizes the corresponding bootstrap residuals (from $500$ replicates) for each component of the estimated parameter vector $\beta_0$; we can see from this figure that treating messages with multiple recipients as multiple single-recipient messages leads to negative bias in the coefficient estimates.

The bias-corrected results of inference on $\beta_0$ are summarized as follows.  Table~\ref{T:group-effects}
\begin{table}[t]
    \tiny
...
\end{table}
shows the estimated group-level (static) effects, and Fig.~\ref{F:reciprocation}
\begin{figure}[t]
    \includegraphics[scale=0.6]{figures/reciprocation-bc}
    \caption{
        \textsc{Reciprocity Effect.}
        Estimated reciprocity effect and standard errors for the
        model described in Section~\ref{S:enron-modeling}.  The solid
        (orange) curve is determined from the 21 estimated coefficients
        for the reciprocity effects at intervals of ``\,$2^l$ hours''
...
\end{figure}
shows the reciprocity (dynamic) effects.  We observe that reciprocity is stronger by orders of magnitude, amplifying the sending rate by between about $10^1$ and $10^3$.  In contrast, group level effects are on the order of $10^{-1}$ to $10^0$.
...
\begin{table}[t]
    ...
    \caption{
        ...
        and the ``Dynamic'' term contains the reciprocity effects.
    }
    \label{T:deviance}
\end{table}
gives an ad-hoc analysis of deviance for the fitted model, showing that group-level (static) effects account for 18\% of the residual deviance and reciprocation (dynamic) effects account for 24\%.  If the model fit perfectly, we would expect the residual deviance to be about equal to the residual degrees of freedom; here, the residual deviance is about $5.9$ times what we would expect.  Scaling the reported standard errors by $\sqrt{5.9}$ is an ad-hoc adjustment for this overdispersion.

\section{Discussion}\label{S:discussion}

Our data analysis example above has demonstrated that static and dynamic effects are both evident in e-mail communication, and we expect this to hold broadly for other types of interactions.  The proportional intensity model with time-varying covariates is a simple, flexible, and well-established model for incorporating these effects into the analysis of interaction data.  As far as we are aware, this is the first time that such an approach has been brought directly to bear on relational data in the form of a network of directed interactions.  With such data becoming more pervasive, and practitioners from across fields eager for new analytical tools, there is an unfortunate tendency to shoehorn disparate forms of data into binary ties between entities.  However, it is important to bear in mind that the observables in such modeling scenarios often come in the form of counts of directed interactions; as shown in~\cite{dechoudhury2010}, thresholding counts to obtain binary adjacency data can dramatically affect both the structure of the resultant network, and the conclusions of subsequent analysis.

In contrast, the foundation of our work is Cox's proportional intensity model and partial likelihood theory, tools which he first introduced almost forty years ago and which have been significantly developed since then~\cite{andersen1993statistical, cook2007statistical, cox1972regression, cox1975partial, fleming1991counting, martinussen2006dynamic}. These tools are used extensively in the context of clinical trials, but require further development to enable their application to directed interaction data.  In this vein, we have extended the associated theory in two directions: first, we have provided results that are asymptotic in time rather than in the population of individuals under study; and second, we have shown that treating multiple-receiver interactions as separate pairwise interactions leads to bias in the corresponding parameter estimates, and we have shown when and how this bias can be corrected.

We are not the first to model dyadic interactions using a point process framework; in a similar vein, Malmgren et al.\ and Heard et al.\ both apply hidden Markov models to communication activity~\cite{heard2010bayesian, malmgen2009characterizing}.  However, neither of these models uses covariate information, and neither posits the problem in continuous time.  Another feature of our model deserving emphasis is that we are using a different baseline intensity for each sender. In the survival analysis terminology, the model is stratified by sender.  Our nonparametric approach takes into account the fact that individuals may have highly idiosyncratic initiating behaviors; in contrast, Malmgen et al.\ use a hidden Markov model to capture individual and seasonal idiosyncrasies~\cite{malmgen2009characterizing}.  The issue of modeling extra-dyadic interactions, which we have also addressed here, has seen less attention in the literature; however, Lunag\'omez et al.~\cite{lunagomez2009geometric} have recent work in the context of graphical models that is relevant.

Finally, we point out similarities to agent-based models from the network analysis literature; see~\cite{snijders2010introduction} for a recent survey.  These approaches and ours both conceive of the data as arising from ego-centric decisions made by the agents under study.  The main difference of approach lies in the fact that we are treating instantaneous interactions rather than ties such as friendship that persist over time.  However, features such as transitivity (the notion that your friends' friends are likely to be your friends) that are typical of such models can easily be incorporated into our framework, simply by adding history-dependent covariates as described in Section~\ref{S:point-process-model} of this article.
...
optimization method described by Nocedal and Write
\cite{nocedal2006numerical}.  This requires an efficient algorithm for
...
Section~\ref{S:point-process-model}, assuming
model~\eqref{E:cox-intensity} and partial likelihood~\eqref{E:log-pl}.
Assume that $|\mathcal{I}| = I$ and $|\mathcal{J}| = J$.

Suppose $(t_1, i_1, j_1), \ldots, (t_n, i_n, j_n)$ is the sequence of observed
messages, where tuple $(t,i,j)$ indicates that at time $t$, sender $i$
interacted with receiver $j$.  Set
$n(i) = \#\{ i_m : i_m = i \} = N_{t_n}(i).$
...
but for large network datasets computations taking time $\Oh(n \, J)$ are
prohibitive.  In the rest of the section we show how to exploit sparsity
...
\[
    \mathcal{\bar J}(i)
        ...
        \}.
\]
Assume $|\mathcal{\bar J}(i)| \le \bar J \ll J$ and that
...
\begin{align}
    ...
    W_{t}(\beta,i)
        &=
            W_{0}(\beta,i)
            +\!\!\!
            \sum_{j \in \mathcal{\bar J}(i) \cap \mathcal{J}_t(i)}\!\!\!
    ...
    \sum_{\substack{t_m \leq t, \\ i_m = i}}
        ...
        =
            ...
            \sum_{\substack{t_m \leq t, \\ i_m = i}}
                d_{t_m}\!(i,j);
\]
computing first term takes time $\Oh(p \, \bar J)$ and computing the second
...
\section{Results from Section~\ref{S:MPLE-consistency}}\label{S:MPLE-consistency-proofs}
...
Using the notation of Theorem~\ref{T:score-fisher}, under assumption
A\ref{A:square-int}, the Lindeberg condition for Rebolledo's Central Limit
...
$\E \Lambda_{\tau_n} = \E N_{\tau_n} = n$, the product of the two also
...
\begin{proof}
Lenglart's Inequality and assumption~A\ref{A:message-times-finite} imply that
...
The sum is bounded by $\frac{16 K^4}{n} \cdot \frac{\Lambda_{\tau_n}}{n}$.
Since $n^{-1/2} K^2 \toP 0$ by assumption A\ref{A:square-int} and
$\E \Lambda_{\tau_n} = n$, the right-hand side of the inequality converges to
...
\section{Results from Section~\ref{S:multiple-receivers}}}\label{S:multiple-recipient-proofs}
