...
\begin{figure}[h]
  \centering
  \subfloat[Observed Count]{
    \makebox{\includegraphics[scale=0.6]{figures/nobs-by-nexp}}}
  \\
  \subfloat[Pearson Residual]{
    \makebox{\includegraphics[scale=0.6]{figures/resid-by-nexp}}}
  \caption{
    Observed count $N_{\infty}(i,j)$ plotted as a function of
    expected count $\hat N_{\infty}(i,j)$ for both models.;
    Pearson residual
    $(N_{\infty}(i,j) - \hat N_\infty(i,j)) / [\hat N_\infty(i,j)]^{1/2}$ plotted as a function of
    expected count $\hat N_{\infty}(i,j)$ for both models.}
\end{figure}
