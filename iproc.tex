...
\[
    N_t(i,j)
        ...
        \}.
\]
For technical reasons, assume $N_0(i,j) = 0$ and that $N_t(i,j)$ is
...
Also note that despite assuming $\mathcal{I}$ and $\mathcal{J}$ to be fixed, our analysis
...
package for R \cite{therneau2009survival}) will not suffice.  In
...
collect more data is to increase the observation time.  Cox \cite{cox1972regression} outlines a proof for general
...
then~\cite{andersen1993statistical, cook2007statistical, cox1975partial, fleming1991counting, martinussen2006dynamic}.
...
Lunag\'omez \emph{et al}.'s \citeyearpar{lunagomez2009geometric} recent work in the
...
the model of~\eqref{E:cox-intensity} and the partial likelihood of~\eqref{E:log-pl}.
