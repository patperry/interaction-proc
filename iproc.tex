...
\begin{abstract}
Network data often take the form of counts of repeated interactions over
time.  Here we consider partial-likelihood inference for the general
case of \emph{directed} interactions involving senders and receivers,
communications data being a canonical example.  We model these data in
continuous time by way of a point process representation, using a Cox
multiplicative intensity model that takes into account covariates that
depend on the history of the process.  We prove consistency and
asymptotic Normality of our estimation procedure as time increases, in
contrast to the fixed-interval setting typical of the clinical trials
literature.  We also address the issue of consistent inference for
interactions with multiple receivers, which has been largely ignored
in the network literature to date.  We show that treating these as
multiple separate interactions leads to biased estimators, and we give
conditions under which this bias can be corrected. We also describe an
efficient implementation of the resulting inference and bootstrap
procedures, and conclude with a data analysis example showing the
effects of reciprocation and group-level preferences on message sending
behavior in a corporate e-mail dataset.
\end{abstract}
...
of interaction data.  As far as we are aware, this is the first time that
such an approach has been brought directly to bear on relational data in
the form of a network of directed interactions.  With such data becoming
more pervasive, and practitioners from across fields eager for new
analytical tools, there is an unfortunate tendency to shoehorn disparate
forms of data into binary ties between entities.  However, it is important
to bear in mind that the observables in such modeling scenarios often come
in the form of counts of directed interactions; as shown
in~\cite{dechoudhury2010}, thresholding counts to obtain binary adjacency
data can dramatically affect both the structure of the resultant network,
and the conclusions of subsequent analysis.

In contrast, the foundation of our work is Cox's proportional intensity
model and partial likelihood theory, tools which he first introduced almost
forty years ago and which have been significantly developed since
then~\cite{andersen1993statistical, cook2007statistical, cox1972regression, cox1975partial, fleming1991counting, martinussen2006dynamic}.
These tools are used extensively in the context of clinical trials, but
require further development to enable their application to directed
interaction data.  In this vein, we have extended the associated theory in
two directions: first, we have provided results that are asymptotic in time
rather than in the population of individuals under study; and second, we
have shown that treating multiple-receiver interactions as separate pairwise
interactions leads to bias in the corresponding parameter estimates, and we
have shown when and how this bias can be corrected.

We are not the first to model dyadic interactions using a point process
framework; in a similar vein, Malmgren et al.\ and Heard et al.\ both
apply hidden Markov models to communication
activity~\cite{heard2010bayesian, malmgen2009characterizing}.  However,
neither of these models uses covariate information, and neither posits
the problem in continuous time.  Another feature of our model deserving
emphasis is that we are using a different baseline intensity for each
sender. In the survival analysis terminology, the model is stratified
by sender.  Our nonparametric approach takes into account the fact that
individuals may have highly idiosyncratic initiating behaviors; in
contrast, Malmgen et al.\ use a hidden Markov model to capture individual
and seasonal idiosyncrasies~\cite{malmgen2009characterizing}.  The issue
of modeling extra-dyadic interactions, which we have also addressed here,
has seen less attention in the literature; however,
Lunag\'omez et al.~\cite{lunagomez2009geometric} have recent work in the
context of graphical models that is relevant.

Finally, we point out similarities to agent-based models from the network
analysis literature; see~\cite{snijders2010introduction} for a recent
survey.  These approaches and ours both conceive of the data as arising
from ego-centric decisions made by the agents under study.  The main
difference of approach lies in the fact that we are treating
instantaneous interactions rather than ties such as friendship that
persist over time.  However, features such as transitivity (the notion
that your friends' friends are likely to be your friends) that are typical
of such models can easily be incorporated into our framework, simply by
adding history-dependent covariates as described in
Section~\ref{S:point-process-model} of this article.
