...
assume each interaction to have a single receiver.
...
process, in particular $x_t(i,j)$ can depend on the history of interactions.  To
...
expressed in terms of weighted means and covariances of the covariates.  The weights are
\begin{subequations}
\begin{align}
    w_{t}(\beta, i,j)
        &=
...
Note that the dependence of these terms on time-varying covariates means that there are no sufficient statistics available to reduce the complexity of partial likelihood maximization.
Therefore, for all but the smallest interaction datasets, maximization of $\log \mathit{PL}_t(\cdot)$ 
requires an approach that is more computationally efficient than
...
collect more data is to increase $t$.  Cox \cite{cox1972regression} outlines a proof for general
...
we have assumed each interaction to involve a single sender and a single
...
related to an extension of our model for directed pairwise interactions, and that the bias introduced by such an
...
$|J_m|$ (i.e., allowing duplicate elements from each $\mathcal{J}_{t_m}(i_m)$), we observe the relation to~\eqref{E:log-pl-multiple-approx} as follows:
\begin{align*}
    \log\big[\!\!
        \sum_{\substack{J \subseteq \mathcal{J}_{t_m}(i_m) \\
              |J| = |J_m|}}\!\!\!
...
Note that when $|\mathcal{J}_{t_m}(i_m)|$ is constant $G_n$ has linear growth,
