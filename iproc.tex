...
model message sending behavior in a corporate e-mail network, with covariates
introduced to account for both static shared traits and dynamic network
effects.  As demonstrated through three comparative analyses, the model
avoids systematic biases that can result when using alternative network models that fail to take into account the specific features of directed interaction data.
...
Given the model fitting procedure and results described above, we may now evaluate the strength of homophily and network effects in predicting the interaction behavior observed in our data.  We also consider comparative analyses based on contingency tables, actor-oriented network models \citep{snijders2010introduction}, and exponential random graph models \citep{anderson1999pstar}.  Each of these methods has seen success in the literature, but none is directly applicable to directed interactions without some form of initial data aggregation.  Ignoring interaction timings, aggregating the data to yield snapshots of static graphs, or thresholding the interaction counts to produce a single directed network can all lead to systematic biases in the analysis.
...
ways in which network effects can manifest themselves in data.  As with our earlier comparison to a contingency table-based analysis, here we compare our corresponding
...
also compare to a static network analysis based on an exponential random graph
model.
...
Figure~\ref{F:enron-ergm} shows the fitted coefficients for the exponential random graph model.  As with the case of the actor-oriented model considered in Section~\ref{sec:actor-oriented} above,  it may again be observed that the estimated network effects agree qualitatively with those of Fig.~\ref{F:enron-net-indicator}.  Specifically, mutuality and transitive triples have positive effects, while cyclic triples have a negligible effect (in contrast to the case of Fig.~\ref{F:enron-actor} from Section~\ref{sec:actor-oriented}).
...
directly, rather than in an aggregated form.  By specifying a model for directed interaction data, we have avoided biases inherent in the aggregation process.
