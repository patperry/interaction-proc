...
\begin{abstract}
Data derived from repeated interactions between entities are becoming more
common, the canonical examples being communication networks.  We model the
interactions as a point process---specifically using a Cox multiplicative
intensity model---to adjust for reciprocation effects and obtain accurate
...
motivating example is estimating gender-level email preferences in a
...
Data derived from repeated interactions between entities are becoming more
common.  The canonical examples are communications networks: either all of
the phone calls or the emails sent between individuals in a community
\cite{eagle2006reality,tyler2005email}. More broadly,
``interaction'' has been construed to include the following:
...
Section~\ref{S:motivating-example} presents a motivating example: testing
for gender bias in a corporate email network.
Section~\ref{S:point-process-model} presents the basic multiplicative
intensity model,
Section~\ref{S:multiple-recipients} discusses extensions for multiple
recipients, and Sections~\ref{S:enron-modeling}~and~\ref{S:gender-bias}
present the results of fitting the model to the email network.  We
prove asymptotic consistency for the single-recipient setting in
Section~\ref{S:MPLE-consistency}, and investigate bias in the multiple
recipient setting in Section~\ref{S:bias-correction}.  
...
to email communication, and we expect this to hold broadly for other
