...
error estimates using Theorem~\ref{T:consistency}.  We exclude messages
with more than 10 recipients---a subjectively-chosen cutoff that includes
99\% of the corpus---because many of these are ``broadcast'' messages and
...
(dynamic) effects.  Reciprocation was stronger by orders of magnitude,
amplifying the sending rate by between about $10^1$ and $10^3$.  Group level
effects, on the other hand were mostly on the order of $10^{-1}$ to $10^0$.
...
\begin{figure}[h]
    ...
    \caption{
        ...
        ``\(\,
            1\{\text{the time since $j$'s most recent message to
                     $i$ is at most $\delta$}\},
        \)''
...
\begin{table}
    ...
    \caption{
        Estimated gender-level sending preferences.
    }
