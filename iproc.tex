...
\begin{theorem}
    Assume messages are sent from senders in $\mathcal{I}$ to
    subsets of the receiver set $\mathcal{J}$ with sender-receiver
    intensity as in~\eqref{E:intensity-multiple}.  Let $\hat \beta_t$
    be the maximizer of $\log \mathit{PL}_t(\cdot)$, defined
    in~\eqref{E:log-pl-multiple}.  Suppose the following conditions
    hold:
    \begin{enumerate}[(a)]
        \item $|\mathcal{M}_t| \to \infty$ as $t\to \infty$.
    \end{enumerate}
    As $t \to \infty$
    \begin{enumerate}[(i)]
        \item $\hat \beta_t \toP \beta$
        \item
    \end{enumerate}
\end{theorem}
