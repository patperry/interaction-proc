...
We wrote custom software in to fit the model using Newton's method.  It took
approximately 70 minutes to fit with 2.4 GHz processor and 4GB of RAM.  Each
bootstrap replicate took approximately 20 minutes to generate and fit, using
the original estimate as a starting point for the fitting algorithm.
...
the full proportional intensity model, which adjusts for dynamic network
effects.  With this model, we can answer the question we set out to ask,
assessing the role of homophily in predicting sending behavior, all else being
equal (i.e., adjusting for network affects).  Notably, homophily appears
present for each main effect (Department, Seniority, and Gender). 
...
important to account for them when making inferences about other covariates.
 ...
 Our final
estimate for Seniority-based homophily is that when choosing between sending
to a Junior executive and a Senior executive, the odds of a Junior sending to
the Junior were roughly $\exp(0.57) \approx 1.8$, and the odds of a
Senior sending to the Junior were roughly $\exp(-0.40) \approx 1/1.5$.  If we
had not adjusted for network effects, then our estimates would have been
$\exp(4.26) \approx 70.8$ and $\exp(-3.21) \approx 24.8$.  Clearly, adjusting
for network affects makes a big difference in quantifying the importance
of homophily.
...
\normalsize
\caption{Estimated coefficients for the ERGM model.  The model also included a
