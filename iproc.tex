...
example shows how network effects and group-level preferences can be incorporated when modeling message sending behavior in a corporate e-mail network.
...
The issues of homophily, network effects, and their interactions are quite
common in the networks literature.  See for example
\citet{mcpherson2001birds, butts2008relational, aral2009distinguishing, snijders2010introduction}, and
references contained therein.  Multiplicity has largely been ignored in the
networks literature, with notable exceptions including
...
or the immediate present.  The data analysis example in
Section~\ref{S:enron-covariates} describes specific covariates for
...
The modeling and inference framework introduced above enables us to directly address several of the key concerns of interaction data analysis raised in Section~\ref{S:introduction}.  To demonstrate the utility of our framework, we investigate a corporate e-mail network consisting of a large subset of the e-mail messages sent within the Enron corporation between 1998 and 2002.  Our goal is to determine which (if any) traits and behaviors are predictive of interactions in these data as follows.

\begin{description}
    \item[Homophily] To what extent are shared traits predictive of interactions behaviors?  Is the existence of a common trait between two actors (gender, department, or seniority) predictive of their e-mail interactions?

    \item[Network Effects] Are network effects, as characterized by past interaction behaviors (dyadic or triadic), valuable in predicting future interaction behaviors?  And if so, which effects dominate?
\end{description}

This analysis is accomplished using the multicast proportional intensity modeling framework developed above.   We employ both static covariates reflecting actor traits, as well as dynamic covariates capturing network effects.  The bootstrap technique introduced in Section~\ref{S:approximation-bias} for multicast interactions is then used to reduce bias in the estimated effects, as well as to demonstrate that our asymptotic approximations are reasonable in this data modeling regime.  We conclude this section with a discussion of the goodness of fit of our model in this setting.
...
\section{Evaluating the strength of homophily and network effects}
...
\toprule
Variate & out & outendow & recip & ctriple & ttriple \\
\midrule
...
$\Oh\big(n \, p^2 + I \, J \, p^2\big)$ or in some cases
$\Oh\big(n \, p^2 + J \, p^2\big)$.  The significance of this latter
expression is that complexity is \emph{linear} in the number of messages and
the number of employees, which means that the algorithm scales is feasible for
large datasets.
...
gradient.  Again suppressing the dependence on $\beta$ and $i$, we have
\[
  V_t = X_t^\trans \diag(\pi_t) X_t - X_t^\trans \pi_t \pi_t^\trans X_t
\]
