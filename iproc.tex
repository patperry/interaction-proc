...
intensity model to capture interaction behavior in social settings.  If we set
$K = 1$, then our $\textbf{send}_t$ covariate becomes an unnormalized version
of Butts' persistance covariate, and the sum $(\textbf{send}_t +
...
covariates are anaogous to our $\textbf{2-send}$, $\textbf{2-receive}$,
$\textbf{sibling}$, and $\textbf{cosibling}$ covariates, although his
definitions are slightly different.
For example, $\textrm{OTP}_t(i,j)$ is
defined as $\sum_{h} \min\big( \#\{ i \to h \text{ in } (-\infty, t) \}, \,
\#\{ h \to j \text{ in } (-\infty, t) \} \big)$.  We prefer our versions of
these covariates, as they quantify more precisely the time-dependence of 
network effects, and the coefficients of our covariates have a more
...
The results of Section~\ref{S:enron-network} provide a detailed view of the ways in which network effects can manifest themselves in data.  To further bolster our confidence in these results, we compared our corresponding parameter estimates to those obtained using related network models.  In this vein, a number of dynamic network models exist in the literature, including
...
time-varying stochastic block model.  Alternative approaches explicitly based on point processes, but excluding network effects and other covariate information, include that of \citet{malmgen2009characterizing}, who model activity at the
level of the individual using a hidden Markov model, and of
\citet{heard2010bayesian}, who work at the level of the dyad, assuming a
piecewise-constant interaction rate.  The closest match to our approach is given by the actor-oriented model of
\citet{snijders2001statistical,snijders2005models}, which we now detail.

The actor-oriented model is designed for a sequence of snapshots $G_1$, $G_2,\dotsc,$ of network
activity, where $G_t$ is an $n \times n$ binary matrix
representing pairwise connectivity between actors at time~$t$.  The model is constructed by
assuming that ties persist in time rather than being instantaneous events; the sequence of networks is considered to be a Markov chain, with the distribution of $G_t$ determined by $G_{t-1}$, and
actors are assumed to change their ties between times $t-1$ and $t$ to maximize a stochastic
utility function that depends on characteristics of the network.  

Essentially, given
that the network is in state $G$, and that actor $i$ is allowed to make
a change, $i$ will change its link to actor $j$ according to
\[
  p(j | i, G) \propto
    ...
    \Big\},
\]
where $T_s$ and $T'_s$ are network statistics;
$\beta_s$ and $\beta_s'$ are unknown coefficients;
$G(i \leadsto j)$ denotes the network obtained either by adding link $i \to j$
if it is absent or removing link $i \to j$ if it is present;
$G \setminus \{ i \to j \}$ denotes the network obtained by removing $i \to j$
if it is present.  Additionally, between observation times $t-1$ and $t$, actor
$i$ is allowed to make changes at rate $\lambda(i)$, specified via another
parametric model.  (See \citet{snijders2010introduction} for a more thorough
introduction.)

For purposes of comparison, we estimated coefficient sets $\beta$ and $\beta'$ in the actor-oriented change probability function $p(j | i, G)$; this function is analogous to the partial likelihood in the Cox proportional intensity model, while the actor rate function $\lambda(i)$ is analogous to the nonparametric baseline
intensity in the Cox model.   We used the \texttt{RSiena} package
\citep{rsiena2011} to specify and fit the actor-oriented model, after binning our e-mail interaction data at regular intervals to obtain network snapshots $G_1$, $G_2$, and $G_3$---with $G_t(i,j) = 1$ if message $i \to j$ was observed in period $t$, and $G_t(i,j) = 0$ otherwise---corresponding to $3$ consecutive four-month periods covering the calendar year 2001.  Approximately 2 hours' time was required to fit the model, and we were unable (for computational reasons) to fit a model to a larger subset of our data.
...
Figure~\ref{F:enron-actor} shows the corresponding fitted coefficients for the
actor-oriented model.  It may be seen that the estimated network effects agree
...
coefficients are larger than the triadic coefficients.  A discrepancy
...
actor-oriented model.  It is also important to note that with network snapshots, in contrast to our approach, it is not possible to separate the $\textbf{2-send}$, $\textbf{sibling}$, and $\textbf{cosibling}$ effects.
Further, the first-order Markov nature of the actor-oriented model means that it is impossible to quantify the time
decay of the dynamic network effects in this setting.
...
\}, 
$
where $T_s(g)$ is a network statistic, for example the number of transitive triples in the graph; i.e., edge sets of the form $\{ (h \to i), (j \to k), (h \to j) \}$.

\cite{anderson1999pstar} provides a detailed survey of exponential random graph models; for our purposes it suffices to note two points:  First, we employed two different reductions of our data to a single directed graph: $G^{(1)}$ and $G^{(10)}$, where
\[
...
$10^5$ following a burn-in period of $10^6$ iterates; an explanation of the
covariates employed follows:
...
\qquad
\subfloat[Estimated group effects for $G^{(10)}$]{
  \centering
  \footnotesize
  \begin{tabular}{lrrrr}
  \toprule
...
\qquad
\subfloat[Estimated network effects for $G^{(10)}$]{
  \centering
  \footnotesize
  \begin{tabular}{lrrr}
  \toprule
