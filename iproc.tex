...
\section{Enron Parameter Estimates}

We fit the model described in Section~\ref{S:enron-modeling} by
maximizing the partial likelihood, and then got Wold-style standard
error estimates using Theorem~\ref{T:consistency}.  
Table~\ref{T:group-effects} shows the estimated group-level (static) effects,
and Fig.~\ref{F:reciprocation} shows the reciprocation (dynamic) effects.
Reciprocation was stronger by orders of magnitude, amplifying the
sending rate by between about $10^1$ and $10^3$.  Group level effects, on the
other hand were mostly on the order of $10^{-1}$ to $10^0$.
...
\begin{figure}[h]
    ...
    \caption{
        ...
        (purple) curve is determined from the 21 estimated coefficients
        corresponding to variables of the form
        ``\(\,
            1\{\text{the time since $j$'s most recent message to $i$ is at most $\delta$}\},
        \)''
        where $i$ is the sender, $j$ is the receiver,
        and $\delta$ is ``\,$2^l$ hours'' for $l$ ranging
        from $-6$ to $14$.  The dashed (orange) curve gives one
        standard error around the estimate as computed from 
        Theorem~\ref{T:consistency}.  If employee $j$'s most recent
        message to employee $i$ was 30 minutes ago, then $i$ sends to
        $j$ at about 200 times his baseline rate.  The reciprocation
        effect decays at a super-exponential rate, but persists for
        1.87 years.
