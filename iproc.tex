...
\begin{figure}
    ...
    \caption{
        The left panel shows a quantile plot of the number of messages
        sent per sender; the numbers range from 1 to 3,844.  The right
        panel shows kernel density estimates of the message sending times
        for each individual; when scaled by the corresponding counts from the
        left panel, these curves give estimates of individual send
        intensities.  Inhomogeneity is present across individuals and
        times.
    }
...
receiver-specific group memberships.  The dynamic covariates depend on the
...
for gender, seniority, and department by F/M, J/S, and L/T/O.  For
...
Let $\hat \beta_t$ be the MPLE of the true parameter vector, $\beta_0$.
...
it turns out to just as difficult to prove as convergence of the
...
\[
    \eta_n
        \leq
            ...
            \varepsilon.
\]
If $|\mathcal{J}_{t_m}(i_m)|$ is constant, then $\eta_n$ is size
$\OhP(1)$.  If, on the other hand, $|\mathcal{J}_{t_m}(i_m)|$ grows
at rate $\omega_P(\sqrt{n})$, then $\eta_n$ is size $\OhP\big(1/\sqrt{n}\big)$
...
\begin{equation}\label{E:log-pl-multiple-as-sum}
