...
\cite{therneau2009survival}.  Unfortunately, with time-varying
covariates, there are no sufficient statistics and most interaction datasets
are too large for the package to handle.  We describe a custom gradient-based
optimization method in  Appendix~\ref{S:implementation} that takes advantage
...
To fit this model to all but the smallest interaction datasets, we require a partial likelihood maximization algorithm that is more efficient than standard tools from survival analysis (e.g., the function \texttt{coxph} from the \texttt{survival} package for R~\cite{therneau2009survival}).  In Appendix~\ref{S:implementation} we outline such an algorithm and analyze its complexity; we have implemented this algorithm as a freely-available stand-alone software package *** \texttt{PackageName} and employ it for all results in this section.

\subsection{Data and Methods}

Our example analysis uses publicly available data from the Enron e-mail corpus~\cite{cohen2009enron}, a large subset of the e-mail messages sent within the Enron corporation between November 1998 and June 2002, and made public as the result of a subpoena by the U.S.~Federal Energy Regulatory Commission during an investigation into fraudulent accounting practices.  We analyze the dataset compiled by~\cite{zhou2007strategies} (see *** \texttt{www....edu}), comprising $21,635$ messages sent among $156$ employees between November 13, 1998 and June 21, 2002, along with the genders, seniorities, and departments of these employees (summarized in Table~\ref{T:employee-summary}).
\begin{table}[t]
...
computing the log partial likelihood and its gradient. For simplicity, we 
...
The algorithm works in space $\Oh(p + \bar J)$.
...
Using the notation of Theorem~\ref{T:score-fisher}, under assumption 
A\ref{A:square-int}, the Lindeburg condition for Rebolledo's Centrial Limit 
...
A\ref{A:square-int}~and~A\ref{A:message-times-finite}
\[
    \left\|
            \frac{1}{n}
            \sum_i
...
The sum is bounded by $\frac{K^4}{n} \cdot \frac{\Lambda_{\tau_n}}{n}$.  
