...
\begin{enumerate}[(1)]
    \item \textbf{The covariates are uniformly bounded.}  That is,
    \[
...
$\mathcal{F}_{\tau_n}$ be the $\sigma$-algebra of events
prior to $\tau_n$. The idea is to change time from the original scale to a scale
on which $\tau_{n} - \tau_{n'}$ is proportional to $n - n'$.
...
$M_{t \wedge \tau_n}$ is a uniformly integrable martingale and
$\Lambda_{t \wedge \tau_n}$ is integrable.  Actually, since $N_{t \wedge \tau_n}$
is a counting process, Lemma~\ref{L:bouned-count-sqint}, shows that
these quantities are square integrable.  Similar properties hold for
$M(i)$, $M(i,j)$, $\Lambda(i)$, and $\Lambda(i,j)$.
