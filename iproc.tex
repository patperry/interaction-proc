...
receivers tabulated over time.  A primary question is to ask which traits and
...
effects.  By modeling the data directly instead of in an aggregated form, the
method avoids systematic biases inherent in the aggregation procedure.
...
this discrepancy is that treating ties as binary forces a negative bias in
...
As a final comparison, we fit an exponential random graph model (ERGM) to the
Enron data.  This class of models---one of the more popular
for estimating the importance of network effects---specifies a probability
distribution for a single directed graph represented by a binary matrix $G$.
Such a model supposes that
\(
  \Prob\{ G = g \} \propto \exp\{ \sum_{s=1}^S T_s(g) \},
\)
where $T_s(g)$ is a network statistic, for example the number of transitive
triples in the graph.  See
\cite{anderson1999pstar} for a detailed survey.

To apply the ERGM to the Enron data, we employed a reduction to obtain
a single directed graph, $G$, defined by
\[
  G(i,j) = 1\{ N_\infty(i,j) \geq 10 \}.
\]
The cutoff value $10$ was chosen by looking for an elbow in the histogram of
$N_\infty(i,j)$ for varying $i$ and $j$.
...
Figure~\ref{F:enron-ergm} shows the fitted coefficients for the model.  Again
it may be observed that the estimated network effects agree qualitatively with
those of Fig.~\ref{F:enron-net-indicator}, as mutuality and transitive triples
have positive effects while cyclic triples have a negligible effect. 
...
an advantage of our approach lies in its ability to model the raw data
directly instead of in an aggregated form.  By dealing with the raw data,
we have avoided biases inherent in the aggregation process.
