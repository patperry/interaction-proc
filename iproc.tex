...
For initiator $i$, receiver $j$, and positive time $t$, define
\[
    N_t(i,j)
        =
        \#\{\text{directed interactions $i\rightarrow j$ in time interval $[0,t]$}\}.
\]
...
Let $\mathcal{I}$ be a set of initiators and $\mathcal{J}$ be a set of receivers.
For each initiator $i$, let $\bar \lambda_t(i)$ be a non-negative predictable
process called the baseline intensity of initiator $i$; let
$\mathcal{J}_t(i)$ be a predictable subset of $\mathcal{J}$ called the
receiver set of initiator $i$.
For each initiator-receiver pair $(i,j)$, let $x_t(i,j)$ be a locally bounded
...
*** This is reasonable/desirable because ***.  Admits partial-likelihood inference which we show to be consistent under suitable regularity conditions/technical assumptions. ***

For technical reasons, assume $N_0(i,j) = 0$.  If
$(t_1, i_1, j_1), \ldots, (t_n, i_n, j_n)$ is the sequence of observed
interactions, then the log partial likelihood at time $t$, evaluated at $\beta$,
is
...
\]
and
$a^{\otimes 2} = a \otimes a = a a^\trans$.
...
\section{Consistency of Maximum Partial Likelihood Inference}\label{S:MPLE-consistency}
...
Under the model of Section ***, the maximum partial likelihood estimator (MPLE) is a natural
...
Typically, one proves consistency of the MPLE by keeping time $t$ fixed
and instead letting $|\mathcal{I}|$ go to infinity
\cite{andersen1993statistical,andersen1982cox,cook2007statistical,fleming1991counting,
martinussen2006dynamic}.  This setting is natural in the context of clinical trial
data, where $\mathcal{I}$ corresponds to the set of patients under study, but does not
meet the requirements typical of interaction data analysis, where $t$ increases while $|\mathcal{I}|$ and $|\mathcal{J}|$ may either be fixed or grow with $t$.  We note that increasing initiator sets can be treated by assuming $|\mathcal{I}|$ is infinite, with $\bar \lambda_t(i) = 0$ for most initiators during the initial part of the observation period; increasing receiver sets can be
handled similarly.

Our proof of consistency relies on a rescaling of time to make the interaction arrivals uniform.  To this end,
...
$\tau_n$. The main idea of the proof is to change time from the original scale to a scale on
...
These technical assumptions are similar to those of Anderson and Gill~\cite{andersen1982cox}, who prove consistency in the clinical trials setting described above, and also investigate specific settings in which their assumptions hold.
...
As observed by Andersen and Gill \cite{andersen1982cox}, the score function $U_t(\cdot)$ evaluated at $\beta_0$ has a simple
...
\section{The Case of Multiple Receivers}\label{S:multiple-receivers}

We have assumed thus far in the article that each interaction involves a single initiator and a single receiver.  While the asymptotic theory we have presented in Section~\ref{S:MPLE-consistency} is thus sufficient to cover the case of strictly pairwise directed interactions such telephone calls between two individuals, we have not yet considered the case in which an initiator interacts simultaneously with multiple receivers---for example, an e-mail message sent to multiple recipients.

In practice, such scenarios are often treated in an ad-hoc manner by treating interactions with multiple recipients as separate pairwise interactions.  In this section we show that such an approach can in fact be viewed as an approximate MPLE fitting method under a natural extension of our Cox proportional intensity model, and that the bias introduced by such an approximation can be quantified and in certain cases corrected.

To this end, introduce $\bar \lambda_t(i ; L)$ as the baseline intensity at time $t$ for
...
\begin{equation}\label{E:intensity-multiple}
    \lambda_t(i,J)
        ...
        .
\end{equation}
This form provides ease of interpretability, in the sense that at time
$t$, changing receiver $j$'s covariates from $x_t(i,j)$ to
$x'_t(i,j)$ results in scaling the rate of interactions $i \rightarrow j$ by
\(
...
The partial likelihood under the model of~\eqref{E:intensity-multiple} is nearly the same as in the single-recipient
...
As a result, the multiple recipient case can be considered as a special case of
...
the theory for the single recipient case (proved in
...
%        \input{tables/gender-effects-naive}
...
\subsection{Proofs of MPLE Results}\label{S:MPLE-consistency-proofs}
