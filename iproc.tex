...
When we perform ad-hoc duplication, we are performing approximate inference
under the multicast model.

When we perform the ad-hoc adjustment, treating multicast interactions
as multiple pairwise interactions using model~\eqref{E:cox-intensity}
instead of model~\eqref{E:intensity-multiple}, we get the approximate partial 
likelihood:
The right hand side of~\eqref{E:log-pl-multiple-approx} can be seen of
an approximation gotten from~\eqref{E:log-pl-multiple} by replacing   Thus, the ad-hoc adjustment can
be seen as leading to approximate inference under model~\eqref{E:intensity-multiple}.
...
In practice, even if we wanted to use model~\eqref{E:intensity-multiple},
the partial likelihood in \eqref{E:log-pl-multiple} involves combinatorially 
many terms and is difficult to optimize.  
...
The multicast case can be considered as a special case of
the single receiver case where $\mathcal{I} \times \mathbb{N}_+$
