...
the rate at which $i$ sends to $j$ will be multiplied by $\exp(0.91) \approx
2.49$, and the rate at which $j$ sends to $i$ will be multiplied by $\exp(1.58)
\approx 4.85$; then, between 30 minutes and 2 hours, the rates will be
multiplied by $\exp(0.42) \approx 1.52$ and $\exp(0.57) \approx 1.77$,
respectively; this proceeds similarly until after 21.3 days, when the rates
will be multiplied by $\exp(0.002) \approx 1.002$ and $\exp(0.002) \approx
1.002$.
...
91\% of the estimated coefficients are within $3$ standard errors of
$0$; even those that are significantly nonzero mostly lie between $-0.05$ and
$+0.05$.  The exceptions are the coefficients for
$\textbf{sibling}_t^{(1,1)}$ ($0.42$),
$\textbf{sibling}_t^{(2,2)}$ ($-0.11$),
$\textbf{cosibling}_t^{(1,2)}$ ($0.25$),
and
