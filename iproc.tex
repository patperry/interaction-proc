...
\begin{align*}
  ...
  \text{\textbf{receive}}^{(k)}_t(i,j)
    &= \#\{ j \to i \text{ in } I_t^{(k)} \};
\end{align*}
for sender $i$ these measure the number of messages sent-to and received-by receiver $j$ in time interval $I_t^{(k)}$.  Likewise, for $k = 1, \dotsc, K$ and $l = 1, \dotsc, K$ we define the triadic effects
\begin{align*}
