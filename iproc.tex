...
standard error for most of the coefficients.  For most of the coefficients,
the bias has the same sign as the estimated coefficient; using the approximate
partial likelihood instead of the true partial likelihood shrinks the
coefficients towards zero.  The root cause of this phenomenon is that
the approximation in~\eqref{E:log-pl-multiple} is worst when the weight
$w_{t_m}(i_m,j)$, considered as a function of $j$, is unbalanced.  This
happens when there is a large spread in the $\beta^\trans x_{t_m}(i_m, j)$
values.  When $|J_m|$ is large, the
...
confirm this, with $17055$ being $12.7$ standard deviances below the mean
