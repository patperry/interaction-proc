...
fit.s <- read.h5out("output/fit-static-full.h5")
fit <- read.h5out("output/fit-dynamic-full.h5")
load("output/boot-full.rda")
...
sink(file = "tables/group-static-full.tex")
print.coefs(coef.s, se.coef.s, TRUE)
sink()

sink(file = "tables/group-dynamic-full.tex")
