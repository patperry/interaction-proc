# tables/group-effects-bc.R
# -------------------------

source("code/boot.R")

file <- "tables/group-effects-bc.tex"
...
ndynamic <- with(dynamic, length(intervals.send) + length(intervals.recv))
bias <- GetBias()
coefs <- coefs - bias$est[-seq_len(ndynamic)]
coefs.se <- sqrt(coefs.se^2 + (bias$est.se[-seq_len(ndynamic)])^2)
rm(ndynamic)
